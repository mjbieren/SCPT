...
{
	size_t sizeFilesFound = 0;
	DIR *pDir;
	struct dirent64 *pDent;
	pDir = opendir(szDirectory);   //this part
	if (pDir != nullptr)
	{
		pvecFiles->clear();
		while ((pDent = readdir64(pDir)) != nullptr)
		{
			//printf("We items in the directory %s", "\n");
			//We only want files, the rest we skip 
			...
			if (patiMatch(szPattern, pDent->d_name))
			{
				...
				sizeFilesFound++;
				if (sizeFilesFound % 100 == 0 )
					printf("Found %zu files that match the extension %s", sizeFilesFound, "\n");
				pvecFiles->push_back(pDent->d_name);
			}
		}
		printf("Closing the directory %s", "\n");
		closedir(pDir);
	}
	else
	{
		printf("Could not open the directory %s %s", szDirectory, "\n");
		return 0;
	}
	printf("Found in total %zu files that match the extension %s", sizeFilesFound, "\n");
...
inline long GetFilteredDirListBoost(const char* szDirectory, const char* szPattern, std::vector<std::string>* pvecFiles)
{
	size_t sizeFilesFound = 0;
	std::string strFileName;


	if (boost::filesystem::is_directory(szDirectory))
	{
		 for (boost::filesystem::directory_iterator itr(szDirectory); itr!=boost::filesystem::directory_iterator(); ++itr)
		 {
			 if (!boost::filesystem::is_directory(itr->path()))
			 {
				 strFileName = itr->path().filename().string();
			 
				 if (patiMatch(szPattern, strFileName.c_str()))
				 {
					 ...
					 if (sizeFilesFound % 100 == 0)
						 printf("Found %zu files that match the extension %s", sizeFilesFound, "\n");
					 pvecFiles->push_back(strFileName);
				 }
			 }
         }
	}
