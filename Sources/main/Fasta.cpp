...
{
	if (bFastaBlockWrite != false && bFastaBlockWrite != true)
...
long CFastaBlock::SetFastaAddonHeader(const char* pFastaAddOnHeader)
{
	if (!pFastaAddOnHeader)
		return FASTABLOCK_SETFASTAHEADER;

	size_t sizeHeader = strlen(pFastaAddOnHeader);
	if (sizeHeader == 0)
		return FASTABLOCK_SETFASTAHEADER;
	...
	m_szFastaHeaderAddOn = new char[sizeHeader + 10];
	memset(m_szFastaHeaderAddOn, '\0', sizeHeader + 10);

	memcpy(m_szFastaHeaderAddOn, pFastaAddOnHeader, sizeHeader);

	long lReturn = SetBoolFastaHeaderAddOn();
	if (lReturn != ERR_NOERROR)
		return lReturn;
...
long CFastaBlock::SetFastaAddOnHeader(const char* pStart, const char* pEnd)
{
	long lSizeDifference = pEnd - pStart;

	if (lSizeDifference <= 0 || lSizeDifference > SIZE_BUFFER_ONE_GIG)
		return FASTABLOCK_SETFASTAHEADERADDON_POINTERDIFFERENCE;
...
long CFastaBlock::GetFastaAddOnHeader(char** ppFastaHeaderAddOn)
{
	if (!m_szFastaHeaderAddOn)
		return FASTABLOCK_GETFASTASEQUENCE;

	*ppFastaHeaderAddOn = m_szFastaHeaderAddOn;

	return ERR_NOERROR;
}

char* CFastaBlock::GetFastaAddonHeader()
{
	return m_szFastaHeaderAddOn;
}


long CFastaBlock::SetBoolFastaHeaderAddOn(bool bFastaHeaderAddOn)
{
	long lReturn = ERR_NOERROR;

	if (bFastaHeaderAddOn != false && bFastaHeaderAddOn != true)
		return 

	m_bFastaheaderAddOn = bFastaHeaderAddOn;

	return lReturn;
}

bool CFastaBlock::GetBoolFastaHeaderAddOn()
{
	return m_bFastaheaderAddOn;
}




long CFastaBlock::SetKeggMatrixBool(bool bKeggMatrix)
{
	long lReturn = ERR_NOERROR;
	if (bKeggMatrix != false && bKeggMatrix != true)
		return FASTABLOCK_SETKEGGMATRIXBOOL;

	m_bFastaKeggmatrix = bKeggMatrix;
	return ERR_NOERROR;

	return lReturn;
}

long CFastaBlock::GetKeggMatrixBool(bool* bpKeggMatrix)
{
	long lReturn = ERR_NOERROR;

	if (m_bFastaKeggmatrix != false && m_bFastaKeggmatrix != true)
		return FASTABLOCK_GETKEGGMATRIXBOOL;

	*bpKeggMatrix = m_bFastaKeggmatrix;

	return lReturn;
}

bool CFastaBlock::GetKeggMatrixBool()
{
	return m_bFastaKeggmatrix;
}

long CFastaBlock::SetKeggMatrixRow(std::vector<char*>* pKeggMatrix)
{
	if (!pKeggMatrix)
		return FASTABLOCK_SETKEGGMATRIX;

	m_pKEGGMatrixRow = pKeggMatrix;

	long lReturn = SetKeggMatrixBool();
	if (lReturn != ERR_NOERROR)
		return lReturn;
...
long CFastaBlock::GetKeggMatrixRow(std::vector<char*>** ppKegMatrix)
{
	if (m_pKEGGMatrixRow == nullptr)
		return FASTABLOCK_GETKEGGMATRIX;

	*ppKegMatrix = m_pKEGGMatrixRow;

	return ERR_NOERROR;
}

std::vector<char*>* CFastaBlock::GetKeggMatrixRow()
{
	return m_pKEGGMatrixRow;
}
...
long CFastaFile::GetMatchingFastaBlock(const char* szMatch, CFastaBlock** ppFastaBlock)
{
	...
	if (bFoundFastaBlock == false)
	{
		printf_s("Could not find Fastablock with the header %s, in fastafile %s %s", szMatch, this->GetFastaFileName(), EOL);
...
long FindMatchingFastaFile(std::vector<CFastaFile*>* pvecGenesFastaFile, const char* szToMatch, CFastaFile** ppFastaFileMatched, bool bDecontaminant, bool bSplitHeader)
{
	long lReturn = ERR_NOERROR;
