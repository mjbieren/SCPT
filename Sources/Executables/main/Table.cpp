...
bool CTable::CompareSecondColumnAlphabetically(std::vector<char*>* a, std::vector<char*>* b)
{

	int iRes = strcasecmp(a->at(1), b->at(1)); // Sort numbers as string (just in case).
	return iRes < 0;
...
void CTable::SortTableRowsAlphabetically(int iColumnToMatch)
{
	...
	else if (iColumnToMatch == 1)
	{
		std::sort(pvecRows->begin(), pvecRows->end(), CompareSecondColumnAlphabetically); //Need to make a template of it
	}
...
bool CompareTableNames(CTable* pTableA, CTable* pTableB)
{
	int iRes = strcasecmp(pTableA->GetTableName(), pTableB->GetTableName());
	return iRes < 0;
}

void SortVectorTablesBasedOnTableNames(std::vector<CTable*>* pvecTables)
{

	std::sort(pvecTables->begin(), pvecTables->end(), CompareTableNames);

	for (size_t idx = 0; idx < pvecTables->size(); idx++)
	{
		pvecTables->at(idx)->SortTableRowsAlphabetically();
	}

}

long FindMatchingTable(char* szMatchingName, std::vector<CTable*>* pvecTables, CTable** ppTableToFind)
{
	if (!szMatchingName)
		return TABLE_FIND_TABLE_TABLE_QUERY_VARIABLE_EMPTY;

	std::vector<CTable*>::iterator iterLowPos;
	std::vector<CTable*>::iterator iterHighPos;

	std::vector<CTable*>::iterator iterBegin = pvecTables->begin();
	std::vector<CTable*>::iterator iterEnd = pvecTables->end()-1;

	char* pTableName = nullptr;
	CTable* pTableToScreen = nullptr;

	CTable TableToMatch;
	bool bFoundTable = false;
	TableToMatch.SetTableName(szMatchingName, szMatchingName + strlen(szMatchingName));

	iterLowPos = std::lower_bound(iterBegin, iterEnd, &TableToMatch, [](CTable* a, CTable* b)
		{
			int iRes = strcasecmp(a->GetTableName(), b->GetTableName());
			return iRes < 0;
		});
	iterHighPos = std::upper_bound(iterBegin, iterEnd, &TableToMatch, [](CTable* a, CTable* b)
		{
			int iRes = strcasecmp(a->GetTableName(), b->GetTableName());
			return iRes < 0;
		});
	...
	if (*iterLowPos)
	{
		if (*iterLowPos != *iterHighPos)
		{
			while (iterLowPos <= iterHighPos)
			{
				pTableToScreen = *iterLowPos;
				pTableName = pTableToScreen->GetTableName();
				if (memcmp(szMatchingName, pTableName, strlen(szMatchingName)) == 0)
				{
					*ppTableToFind = pTableToScreen;
					bFoundTable = true;
					break;
				}
				iterLowPos++;
	...
	if (bFoundTable == false)
		return TABLE_FIND_TABLE_IN_TABLE_VECTOR_DOES_NOT_EXIST;

	return ERR_NOERROR;
}

long CreateRowAndClearColumn(std::string strStringToClearWith, size_t sizeNumberOfColumns, std::vector<char*>* pRow)
{
	long lReturn = ERR_NOERROR;

	size_t sizeStringToClearWith = strStringToClearWith.size();
	if (sizeStringToClearWith == 0) //It's empty
		return TABLE_STRING_TO_CLEAR_ROW_AND_COLUMN_EMPTY;


	char* pColumn = nullptr;

	for (size_t idx = 0; idx < sizeNumberOfColumns; idx++)
	{
		pColumn = new char[sizeStringToClearWith + 5]; 
		memset(pColumn, '\0', (sizeStringToClearWith + 5));
		memcpy(pColumn, strStringToClearWith.c_str(), (sizeStringToClearWith + 5));
...
long ReplaceColumnData(std::vector<char*>** ppRow, size_t idxColumnToReplace, char* szColumnReplacing)
{
	long lReturn = ERR_NOERROR;

	//First Check if the szColumnReplacing has content in it.
	if (!szColumnReplacing || strlen(szColumnReplacing) == 0)
		return TABLE_REPLACE_COLUMN_NEW_COLUMN_EMPTY;

	std::vector<char*>* pRow = *ppRow;
	char* pCurrentColumn = nullptr;

	pCurrentColumn = pRow->at(idxColumnToReplace);

	if (!pCurrentColumn) //if it doesn't exist
	{
		pRow->at(idxColumnToReplace) = szColumnReplacing;
	}
	else
	{
		delete pCurrentColumn;
		pRow->at(idxColumnToReplace) = szColumnReplacing;
	}
