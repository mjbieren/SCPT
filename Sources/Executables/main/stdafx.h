...
#define MOVE_PTR_TO_PREVIOUS_SPACE(pValue) while(*pValue && *pValue != ' ' && *pValue != '\t') pValue--;
...
#define MOVE_PTR_TO_ASCII_CHAR(pValue, cValue) while(*pValue && *pValue != cValue) pValue++;
...
#define MOVE_PTR_TO_PREVIOUS_ALPHA2(pValue) while(*pValue && (*pValue < 'a' || *pValue > 'z') && (*pValue < 'A' || *pValue > 'Z')) pValue--;
